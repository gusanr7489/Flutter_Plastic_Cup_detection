...
framework = 'tflite'
weights_path = './checkpoints/test730-tiny-fp16.tflite'
size = 416
tiny = True
model = 'yolov4'
...
def get_detections_by_image_files():
  ...
  # loop through images in list and run yolov4
  for count, image_path in enumerate(image_path_list):
    ...
    try:
      ...
      for name in image_path_list:
        os.remove(name)
      abort(404, "it is not an image file. try jpg or png")
...
def get_image_by_application():
  ...
  for i in range(valid_detections[0]):
    print('\t{}, {}, {}'.format(class_names[int(classes[0][i])],
                                np.array(scores[0][i]),
                                np.array(boxes[0][i])))
    if class_names[int(classes[0][i])] == 'reusable':
      flag = True
    response.append({
          "class": class_names[int(classes[0][i])],
          "confidence": float("{0:.2f}".format(np.array(scores[0][i]) * 100)),
